    ...
    function (record, log, search, format) {
        ...
        function execute(context) {
            try {
               ...
               var AfterthreeDays = new Date(today.setDate(today.getDate() + 3));// To set the after days to present date
               log.debug('Afterthreedays:', AfterthreeDays);
                var billPaymentSearch = search.create({// Created Saved search billpayment
                    type: "vendorpayment",
                    filters:
                        [
                            ["type", "anyof", "VendPymt"],
                             "AND",
                             ["trandate", 'onorbefore', format.format({value: AfterthreeDays, type: format.Type.DATE})],
                             "AND",
                             ["mainline", "is", "T"]
                        ],
                    columns: ['internalid']
